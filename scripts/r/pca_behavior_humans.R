...
# Loading functions:
source ("/home/kadomu/projects/20200421_pca_behavior_elena/scripts/graph_parameters.R")
...
file <- "/home/kadomu/projects/20200421_pca_behavior_elena/data/tbl_PCA_all_variables.csv"
# file <- "/home/kadomu/projects/20200421_pca_behavior_elena/data/tbl_PCA_main_variables.csv"
data <- read.csv ("/home/kadomu/projects/20200421_pca_behavior_elena/data/tbl_PCA_main_variables.csv",
                  dec=".", 
                  sep=",",
                  na.strings = "NA",
                  stringsAsFactors = F)
...
p_circle_plot <- ggplot(circle_plot) + 
  ...
  #                  geom_text (data=circle_plot, aes (x=Dim.1, y=Dim.2, label=labels_v, hjust=1.2), show.legend = FALSE, size=5) + 
...
ca_reinstatement_var <- ggplot (ca2plot_col, aes(x=Dim.1, y=Dim.2, colour=varGroup)) + 
  ...
  geom_text (aes(label=var), vjust=-0.5, hjust=1, size=5, show.legend = F) +
  theme(legend.key=element_rect(fill=NA)) +
  labs(title = "CA reinstatement raw data:\nVariables\n", x = paste("\nDim 1 (", var_dim1, "% of variance)", sep=""), 
       y=paste("Dim 2 (", var_dim2, "% of variance)\n", sep = "")) +
  #                           guides(colour = guide_legend(override.aes = list(size = 10)))+
