...
library("lme4")
data_short_df <- subset(data, 
                        taxon == "Verrucomicrobia" & behavior_idx == "Persistence_EP")
load("~/Downloads/dragons.RData")

mountain.lm <- lm(testScore ~ bodyLength2 + mountainRange, data = dragons)
summary(mountain.lm)

behavior.lm <- lm(index ~ microbio_rel_ab + Grouping , data = data_short_df)
sm <- summary (behavior.lm)

conts <-  mcp(group = c(
   "wt.wo_DOX.15 - wt.NA.0 = 0"))
conts <-  mcp(group = c(
  "Non-Addict - Addict = 0"))
...
pvals <- coef(sm)[paste0("Grouping", sam),4]
pvals

data.frame(Estimate=coef(sm)[paste0("Grouping", sam), 1], 
           confint(behavior.lm)[paste0("Grouping", sam),, drop=F])
...
summary(behavior.lm)
head(data_nest[[1]])
lmm_fun <- function(df) cor.test(df$microbio_rel_ab, df$index, method = cor_method) %>% tidy()
