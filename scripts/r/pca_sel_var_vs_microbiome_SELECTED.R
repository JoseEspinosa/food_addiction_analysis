...
pca_dim_filt <- pca_dim [,c(1,2,8)]

## Merge tables
# pca_miRNA_merged <- merge (miRNA_clr, pca_dim, by= "mouse_id")
pca_miRNA_merged <- merge (miRNA_clr, pca_dim_filt, by= "mouse_id")
...
data <- gather(pca_miRNA_merged, miRNA, value, first_miRNA:last_miRNA)%>%
        gather(pca_dim, pca_coord, Dim.1:Dim.2)
...
title_p <- paste("Correlations between principal component and ", 
                  "miRNAs expressions","\n")
title_p <- ""
...
axis_text_size_x <- 16; size_p_values <- 5; angle_reg <- 0; microbio_set <- "selected"
width_p <- 16; height_p <- 12

ggsave (hm, file=paste0(out_dir, "fig_sup_Jose_pca_vs_miRNA", extension_img),
        width = width_p, height = height_p, dpi=dpi_q)

# All miRNAs
axis_text_size_x <- 14; size_p_values <- 4; angle_reg<-270; microbio_set <- "all"
